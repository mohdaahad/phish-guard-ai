export default function WhoProgramSection() {
  const criteria = [
    "Are solving a clear and meaningful problem",
    "Are ready to move from idea to execution",
    "Who want strategic support, not just funding",
    "Have a unique or differentiated startup idea"
  ];

  return (
    <section className="professional-whoprogram-section">
      {/* Background */}
      <div className="absolute left-0 top-10 w-full h-[240px] bg-[#EBF7FF]" />
          ...
          <div className="professionals-whoprogram-criteria">
            {criteria.map((criterion, index) => (
              <div key={index} className="professionals-whoprogram-item">
                ...
                <p className="professionals-whoprogram-text sp-body-medium">
                  {criterion}
                </p>
              </div>
            ))}
